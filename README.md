...
### v0.4.4 优化
- ⌨️ 输入框支持上下键切换历史消息，补齐类似 CLI 的快速回输体验
- ✏️ 用户消息新增编辑按钮，可一键回填到输入框后修改重发
- 🧩 Models 页签新增 API type 下拉预设，并保留自定义输入兼容未知类型
...
| **会话管理** | 新建会话、恢复历史会话、通过 Tree 抽屉从更早的用户消息回溯分叉、内联重命名、导出 HTML、关闭 Agent——通过侧边栏或右键菜单即可完成。 |
...
Markdown 渲染 + 流式输出、工具调用详情、模型/思考等级/上下文/缓存命中率状态栏（与 pi CLI 口径对齐）、Git 分支选择器、操作按钮（New Session · Stop · Reload · Restart）。
...
内置斜线命令建议面板（带功能说明），配合右侧历史会话抽屉和 Tree 回溯抽屉，快速浏览、恢复并从更早的用户消息分叉对话。
